...
func init() {
	...
	TestReleaseBundles = flag.Bool("test.rb", false, "Test release bundles")
...
func GetNonVirtualRepositories() map[*string]string {
	nonVirtualReposMap := map[*bool][]*string{
		...
		TestReleaseBundles:     {&RtDevRepo, &RtProdRepo},
	}
...
func GetBuildNames() []string {
	buildNamesMap := map[*bool][]*string{
		...
		TestReleaseBundles: {&RbBuildName1, &RbBuildName2},
	}
...
func AddTimestampToGlobalVars() {
	...
	RbBuildName1 += uniqueSuffix
	RbBuildName2 += uniqueSuffix
	RbRbName1 += uniqueSuffix
	RbRbName2 += uniqueSuffix
	RbRbName3 += uniqueSuffix
